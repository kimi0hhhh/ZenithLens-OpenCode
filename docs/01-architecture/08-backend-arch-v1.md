...
supersedes: —
created: 2026-09-12
reviewers: [backend-dev, dev-lead]
gate: G-AR-04
---
...
- **原子性**：一次 `value_fund` 要么返回一个确定 mode，要么返回 `unknown`；**同一基金同日只允许一个 mode**。
...
# 训练：加噪 K=4，每维以 p=0.25 概率 ±1 档
for sample in train:
    for k in 1..K:                                  # K=4 个加噪副本
        for d in {r2,om,vr}:
            if rand() < 0.25: cell_d = clamp(cell_d + choice(-1,+1), 0, 4)
        add cell → label(up = close[next_H] > close[t])

# 出值：收缩到训练段全局涨率
n      = obs[target_cell].count
n_up   = obs[target_cell].up_count
train_rate = 训练段整体涨率
if n >= MIN_N(15):
...
- `ddsm_state` 返回**未加噪的真实档位**三元组（用于展示与台账）。
...
if crowd >= quantile_train(crowd, 0.60):        # 处训练段最拥挤 40%
    veto_crowd = True

# ② KLOW2：下影线占振幅比，最不利 40% 否决
klow2 = (min(open,close) - low) / (high - low)   # high==low 时视为缺失→不否决并记数据缺失
if klow2 <= quantile_train(klow2, 0.40):         # 处训练段最不利 40%
    veto_klow2 = True
```
...
```
if veto_crowd or veto_klow2:   signal = "abstain"
...
| 窗口 | 定式（PRD F-06） | 说明 |
|---|---|---|
| T+1 | 门 + DDSM + 双否决 | 完整链；`signal` 仅在 T+1 出口产生 |
| T+3 | 仅触发做多 | 门命中即视为方向做多，不叠加双否决 |
| T+15 | 门 + DDSM | 门 + DDSM 出值，不叠加双否决 |

- 标签定义：`up = 1 若 close[T+H] > close[T]`（H=1/3/15）。
- `p_up` 为 T+1 的收缩概率；T+3/T+15 返回各自窗口的 P/方向与命中统计。
...
```
forecast(window):
  pool = [h for h in holdings if h.tier in ("A","B") and pred[h].window[window].open]
  # 规则①：只算开口（signal==up / 该窗口定式开口）；灰从分子分母同剔
  # 规则②：C/D 档不参与
  w_i   = market_value(h_i)                          # 市值权重
  weighted_p      = Σ(w_i × p_i) / Σ(w_i)             # 无开口 → null
  coverage_value  = Σ(w_i)
  total_value     = Σ(all holdings market_value)
  coverage_ratio  = coverage_value / total_value       # 覆盖市值占比
  excluded_ratio  = 1 - coverage_ratio                 # "其余 X% 不表态"
```

- 无任何开口 → `weighted_p=null, open_count=0`，界面"今日无开口"。
