...
- 每格：颜色由 `color_band` + 图例；悬停显示 `state`、`n`（naive_n/ddsm_n）、`P`（naive_p/ddsm_p）。
...
- **主消费方**：`frontend-dev`。需其执行：按本目录落 7 视图与全部组件；`ValBadge` 读 `valuation_mode`、`UnknownCell` 阻断 `+0.00%`、`PrivacyMask` 只作用展示；所有请求经 `api.js`；§9 的 40 项组件按 §4 逐项落地，**缺一即 G-FE-01 不通过**。
