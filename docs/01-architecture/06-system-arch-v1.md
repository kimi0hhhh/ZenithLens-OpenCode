...
supersedes: —
created: 2026-09-12
reviewers: [dev-lead, backend-dev, frontend-dev]
gate: G-AR-02
---
...
| 实时估值 | 腾讯实时 `qt.gtimg.cn` / 天天基金估算 / 新浪 `hq.sinajs.cn` | 盘中/盘后按需拉取；不在磁盘长存，仅当次响应 | 不适用 | 内存（当次）+ 当日快照 `data/snapshot/valuation-YYYYMMDD.json` |
...
ZenithLens-OpenCode/
  ...
  data/                      # ★ 运行期数据（不入版本库）
    ...
    snapshot/
      valuation-YYYYMMDD.json# 当日估值快照（含各档 mode）
...
| 官方净值回填 | 写入 `nav.*`，**不改已入账估值的 `valuation_mode`** | 可区分"既成事实 vs 估算" | 否 |
| 引擎未运行 | 预测/台账接口返回缓存或空态 | 引擎页 HUD"离线"+最近成功时间；其他页标"数据时间" | 否 |
