...
9. 变更纪律：v2/v3/v4 的逐条追加见 §8、§9；前后端须按 v4 实现，v3 及更早已冻结为历史（`09-api-contract-v3.md` 等）。
...
| `factor_cap` | integer/null | 个 | 整数 | **v3**：该盘因子上限（正式盘：否决位 ≤4、打分位 ≤3；其他 null） |
...
| `window_weights` | array[object] | — | — | **v3**：六窗权重 `[{d15:8},{d30:22},{d60:15},{m6:10},{m24:30},{full:15}]`（合计 100） |
| `window_min_days` | array[object] | — | — | **v3**：准入度分母 `N_w`：`{d15:5,d30:10,d60:15,m6:30,m24:60,full:120}` |
| `license_lines` | array[object] | — | — | **v3**：牌照四线 `[{candidate:50},{probation:60,window_k:0.5},{renew_t2:70,window_k:0.7},{full_t3:80,window_k:0.7}]` |
...
| `naive_p` | number/null | 概率 | 4 位 | 研究口径 P；empty 时 **null**（无估计值，禁止渲染为 0 或 50%） |
| `ddsm_n` | integer | 样本 | 整数 | 还原离散计数（生产口径，加噪后 ×5） |
| `ddsm_p` | number/null | 概率 | 4 位 | 生产口径 P（加噪+收缩）；empty 时 null |
...
| 字段 | 类型 | 单位 | 精度 | 口径说明 |
|---|---|---|---|---|
| `groups` | array[object] | — | — | 每项 `{confidence, count, market_value, weight_ratio}`（按 §5.2 置信度） |
...
- 错误：`E_IO`。
- 口径：读取时按 `(date, code)` 左连接 `backfill.jsonl`；只读，**无编辑/删除端点**（PRD G10）；v4 归档不影响"当日首次口径"可读（A-10）。
...
- 口径：每格必须同时给 `naive_p`/`naive_n` 与 `ddsm_p`/`ddsm_n`；`status=empty` 的格**必须显式返回**（两 P 均 null），前端渲染斜纹"无样本"，**禁止**渲染为 0 或 50%；`comparison` 四指标齐备；naive 用**未加噪真实档位**计数，ddsm 用加噪+收缩结果（A-05/A-07）。**双口径**：`production_layer=ddsm`（默认展示）、`research_layer=naive`（默认折叠），前端初始层取 `default_layer`，`dual_view_note` 必展示。
...
- `/valuations/precision` 的 `groups` 元素仍为 `{confidence, count, market_value, weight_ratio}`，v3 **只追加** `mode_distribution`。
