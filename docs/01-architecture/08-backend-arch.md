...
9. **v5 销项（S4 评审 · 契约 v5）**：`build_cube` 的 `naive_p` 明确为**研究口径原始频率**（`naive_n>0` 即给 `naive_up/naive_n`，**不受 MIN_N 门控**；`MIN_N` 只门控生产口径 `ddsm_p`），并新增 `naive_low_sample`（`naive_n<min_n`）供前端标注低样本——消除「研究层整片无样本致对照卡名存实亡」（SHOULD-5）。`ddsm_p` 收缩口径与其余预测核心链**不变**。
...
| `CubeCell` / `CubeSlice` / `StateCube` / `CubeComparison` | `state[3], vr_layer, naive_p, naive_n, naive_low_sample, ddsm_p, ddsm_n, status, color_band` / `production_layer, research_layer, default_layer, dual_view_note` | `core/cube.py` |
...
build_cube(code):
  ...
  # naive_p = naive_up/naive_n（naive_n>0 即给；研究口径不设 MIN_N 门，v5/SHOULD-5）
  # naive_low_sample = (naive_n < MIN_N)（研究口径低样本标注；empty 格亦 true）
  # ddsm_p  = (ddsm_up + 10 × train_rate) / (ddsm_n + 10)（生产口径，受 MIN_N 门控）
  return StateCube(
...
- **空样本格必须显式输出**：`status="empty"`、`naive_p=null`、`ddsm_p=null`，不得省略、不得填 0。**v5 判定式**：`status="filled" ⟺ naive_n>0`，`status="empty" ⟺ naive_n=0`（研究口径原始频率只要有一个样本就给值，低样本另由 `naive_low_sample=true` 标注）。
...
### 10.1 v5 销项实现落点（SHOULD-5：naive_p 研究口径）

| 裁定 | 实现落点 | 验收断言 |
|---|---|---|
| naive_p 研究口径原始频率（不受 MIN_N） | `core/cube.build_cube`：`naive_p = naive_up/naive_n`（`naive_n>0` 即给）；`naive_n=0` → null；新增 `naive_low_sample=(naive_n<min_n)` | `test_engine_contract`：构造 `0<naive_n<15` 的格 → `naive_p` 非 null 且 `naive_low_sample=true`；`naive_n=0` → 两 P 均 null、`status=empty`、`color_band="no_sample"` |
...
- [x] **v5 SHOULD-5**：`naive_p` 研究口径不设 MIN_N（`naive_n>0` 即给原始频率）+ `naive_low_sample` 低样本标注 + `status` 判定式（§9.1、§10.1）

## 下游交接

- **主消费方**：`backend-dev`。需其执行：按 `core/` 模块落实现，预测链参数与估值五档**逐字对齐**本文件；接口字段严格对齐 `09-api-contract` **v5**（G-BE-02 红线：字段名不一致 FAIL）。v3/v4：按 §9/§10 新增 `core/cube.py`/`arena.py`/`governance.py` 与派生视图，并落地 A-07~A-10。**v5**：按 §10.1 调整 `build_cube` 的 `naive_p`（去 MIN_N 门控）+ 补 `naive_low_sample`；DDSM 生产链与其余口径不变。
