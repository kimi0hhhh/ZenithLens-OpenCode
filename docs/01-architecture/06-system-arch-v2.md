...
3b. **v2 裁定（G-AR-06）**：当日估值改 INSERT-ONLY（`data/snapshot/valuation_history.jsonl` 为审计源，`valuation-YYYYMMDD.json` 仅为可重建缓存，A-01）；未知持仓成本不计入 `total_value`（A-02）；组合方向加最小覆盖门槛（A-03）；T+3/T+15 未验证不出方向（A-04）；否决层缺失即弃权（A-05）；引擎离线区分快照/旧快照/无快照，禁止空态承载故障（A-06）。
4. **通信不用 WebSocket**：数据 T+1 更新，无实时推送需求；任务进度用 12 秒内轮询，成本可忽略。
5. 失败降级矩阵（§6）与边界（§7）是本文最硬的约束，任何实现若绕过即触发 G-AR-02 退回。
...
| 预测/台账 | 本地引擎计算 | 收盘时点开口写盘；到期回填 | INSERT-ONLY | `data/ledger/signals.jsonl` |
...
前端 GET /api/v1/portfolio/forecast                    ──►  读当日预测快照（无则返回空态，不现算）
...
| 引擎未运行 | 有旧快照 → 返回旧数据 + `engine_status=offline`（陈旧态）；无任何快照 → `E_ENGINE_OFFLINE`（error 态）；**禁止**用空态承载 | 引擎页 HUD"离线"+最近成功时间；预测页"引擎未运行，展示 <date> 快照"或"引擎未运行"错误态（A-06） | 否（有旧快照） |
...
到期检查（每次启动 + 每日收盘后）：
  for each ledger line where t{n}_real is null and 已到期:
      ...
      仅 append 一条回填补丁 或 就地补字段？ → 决策：jsonl 不可就地改
      方案：新增 backfill.jsonl，按 (date,code) 关联；读取时合并
```

> 注：JSONL 不可变，回填不改原行；用独立 `data/ledger/backfill.jsonl` 承载 `t*_real`，读取台账时按 `(date, code)` 左连接。这样"INSERT-ONLY"对原信号文件成立，回填文件同样 append-only。
...
- **主消费方**：`07-frontend-arch` / `08-backend-arch`。需其执行：分别按展示层/领域层边界落目录与模块；文件字段以 `09-api-contract` 为准。
- **次消费方**：`dev-lead`。需其执行：G-DL-02 构建时核验"零第三方依赖 + Python 3.8 + 端口自适配 + 台账 append-only 回填文件方案"。
- **残留风险**：
  ...
  5. **`valuation_history.jsonl` 体积管理**：按刷新频率线性增长，需在 S3 评估归档/压缩策略（责任人：backend-dev，期限：S3）。
- **未覆盖**：前端目录与组件树（`07-frontend-arch`）、后端算法与模块（`08-backend-arch`）、接口逐字段（`09-api-contract`）。
