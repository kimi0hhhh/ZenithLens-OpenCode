...
>
> **v7 判定方法（因子候选提交与回测接口 + Factor/Scorecard 字段 · OC-FACTOR-ARCH）**：为"每来一个新因子怎么回测/准入"提供**可复用机制**（机制正文见 `21-factor-governance.md`）——① 新增 **2 个接口**（54 `POST /factors/candidates` 提交因子定义并触发回测、55 `GET /factors/candidates/{candidate_id}` 取记分卡）（§4、§6.8）；② 新增共享对象 **`Factor`（§5.35）/ `FactorDefinition`（§5.36）/ `Scorecard` + `ScorecardWindow`（§5.37）**，补齐 lift 六窗、`k_lift`、权重、准入度、总分、F1/F2/F3、牌照、盘位、共线性、Δ、样本/独立触发日；③ 新增 v7 枚举 8 个（§1.3）。**v7 只加不改**：不改任何既有字段名/类型，不改 `factor_book` 六本与四盘枚举，晋升/淘汰**不**经本端点（由治理周期与状态机驱动）。追加台账见 §12。
>
> **v8 判定方法（覆盖度档位补全与主动基金持仓穿透 · OC-COV-ARCH2）**：把后端 `14-api-impl-report` v8 §0f 实测的**代理映射补全 + 持仓穿透**同步进产品法律（逐条纪要见 `20-arch-verdict-r4.md`）——① `HoldingRow` 新增 **`penetration_trace`**（object/null，主动基金前十持仓→个股→行业 ETF 加权穿透记录；字段口径见 §5.1、共享对象 §5.38）；② `CoveragePanel` 新增覆盖计数 **`no_proxy_count`**（无代理只数）/ **`penetrated_count`**（经穿透补全只数）（§5.19）；③ 新增 **A/B/C/D 分档判定表**与 **`penetration_trace.confidence` 枚举**口径（§1.3、§5.19）；④ 实测 D 档由 **12/22（54.55%）压到 0**，穿透样例 `025500→512480`、`021528→515880`，全部标「待验证」。**v8 只加字段/对象/枚举，不改任何既有字段名、类型或语义**；穿透结果一律 `confidence` 非 `high`、`status` 待验证，不得当已验证消费。追加台账见 §13。

## 摘要

1. 共 **55 个接口**，分 9 个域：元信息 / 持仓 CRUD / 实时估值 / 预测 / 组合加权 / 复盘 / 台账 / 因子 / 引擎 / 任务 / 设置。其中内部接口 2 个（`POST /ledger/entries`、`POST /ledger/backfill`），**面向 UI 53 个**（v1/v2 的 38 + v3 的 15 + v7 的 2）。
...
9. 变更纪律：v2/v3/v4/v5/v6/v7 的逐条追加见 §8、§9、§10、§11、§12；前后端须按 v7 实现，v6 及更早已冻结为历史（`09-api-contract-v6.md` 等）。
...
12. **v7 裁定（因子候选提交与回测 · 可复用机制，2026-09-12）**：① 新增 2 个接口——`POST /factors/candidates`（提交 `FactorDefinition` 并触发回测，202）、`GET /factors/candidates/{candidate_id}`（取 `Scorecard`）（§6.8）；② 新增共享对象 `Factor`（§5.35，在册因子全量记录）、`FactorDefinition`（§5.36，提交请求体）、`Scorecard`/`ScorecardWindow`（§5.37，六窗 lift/`k_lift`/权重/准入度/总分 + F1/F2/F3 + 牌照/盘位/共线/Δ/样本）；③ 新增枚举 8 组（§1.3）。机制正文见 `21-factor-governance.md`；**只加不改**，晋升/淘汰不经本端点。
13. **v8 裁定（覆盖度档位补全与主动基金持仓穿透，2026-09-12）**：① `HoldingRow` 新增 `penetration_trace`（object/null，见 §5.38）；② `CoveragePanel` 新增 `no_proxy_count`（无代理只数，本期实测 0）/ `penetrated_count`（经穿透补全只数，本期实测 2）；③ 新增 A/B/C/D 分档判定表（A 行业 ETF 直接 / B 代理已映射且可取日线 / C 拟用·近似或穿透得出待验证 / D 无映射且未穿透）与 `penetration_trace` 的 `confidence` 枚举口径；④ 实测分档 `A10/B0/C0/D12` → `A2/B13/C7/D0`（D 占 0%）。**只加不改**，穿透结果一律「待验证」，不得当已验证消费。
...
**v7 追加枚举（穷举；仅新增，不改既有取值；口径正文见 `21-factor-governance.md` §1/§6）**：

| 枚举名 | 取值 | 中文/口径 |
|---|---|---|
| `factor_kind` | `binary_signal` / `score` | 逐日二元信号 / 逐日打分 |
| `factor_orientation` | `higher_bullish` / `higher_bearish` | 值越大越看涨 / 值越大越看跌（回测取负） |
| `factor_role_hint` | `veto` / `score` | 否决位 / 打分位 |
| `candidate_status` | `submitted` / `backtesting` / `done` / `failed` | 已提交 / 回测中 / 已完成 / 回测失败 |
| `backtest_verdict` | `admitted_to_arena` / `rejected_f1` / `rejected_f2` / `rejected_f3` / `insufficient_data` / `duplicate` | 进有效影子盘 / 未过 F1 / 未过 F2 / 未过 F3 / 样本不足 / 同定义已回测 |
| `normalization_mode` | `none` / `zscore_train` / `quantile_train` | 不归一 / 训练段 z 标准化 / 训练段分位 |
| `param_type` | `int` / `float` / `bool` | 参数类型 |
| `expression_op` | `and` / `or` / `not` / `gt` / `ge` / `lt` / `le` / `eq` / `add` / `sub` / `mul` / `div` / `neg` / `abs` / `log` / `shift` / `rolling_mean` / `rolling_std` / `rolling_min` / `rolling_max` / `rolling_quantile` / `rank_causal` / `zscore_causal` / `corr_causal` / `ewma` | 表达式 AST 运算符穷举（布尔/比较/算术/因果时序；**禁负 `shift`、禁未来窗口**） |

> `factor_kind`/`factor_orientation`/`factor_role_hint`/`normalization_mode`/`param_type` 为因子定义枚举；`candidate_status`/`backtest_verdict` 为回测流程枚举；`expression_op` 为声明式表达式算子集。新增列/算子须版本 +1。

**v8 追加枚举（穷举；仅新增，不改既有取值；口径正文见 §5.19/§5.38）**：

| 枚举名 | 取值 | 中文/口径 |
|---|---|---|
| `penetration_method` | `eastmoney_f10_top10_weighted` | **单值枚举**：抓取天天基金 F10 前十持仓、按个股→行业 ETF 映射加权（禁止静默扩展新穿透算法） |
| `penetration_status` | `pending_validation` / `no_mapping` / `no_report` | 待验证（穿透得出主代理、未过走前验证） / 无映射（前十有数据但无标的命中「个股→ETF」映射） / 无报告（F10 前十未取得） |

> `penetration_trace.confidence` **不新增枚举**，复用既有 `confidence`（`high`/`mid`/`low`/`unknown`）；穿透结果一律 `low` 或 `unknown`，**不得为 `high`/`mid`**。`penetration_status` 的中文展示文案由后端给 `status_label`，前端不得硬编码（同 `valuation_mode`/`mode_label` 口径）。
...
| **持仓穿透** `penetration` | 对无静态代理的主动全市场基金，取 F10 前十持仓→个股→行业 ETF 加权，得出**主代理**的推导过程（结果待验证） | 重仓加权、代理近似、覆盖度 |
| **主代理** `proxy`（穿透语境） | 穿透加权篮子中**权重最高**的行业 ETF，作为该主动基金的拟用代理（`confidence` 非 high） | 代理已验证、直接对应 |
...
**v7 新增接口（54~55，因子候选提交与回测；详见 §6.8）**

| # | 方法 | 路径 | 调用方 | 用途 |
|---|---|---|---|---|
| 54 | POST | `/api/v1/factors/candidates` | 因子页/挖掘流程（提交候选） | 提交 `FactorDefinition` 并触发回测（异步任务） |
| 55 | GET | `/api/v1/factors/candidates/{candidate_id}` | 因子页（记分卡） | 取 `Scorecard`（六窗/评分链/准入/牌照/盘位） |

> 合计 **55 个接口**（含内部接口 20/21）；**面向 UI 53 个**。54/55 只做"提交 + 取记分卡"，**不承担晋升/淘汰**（仍由治理周期与状态机驱动）。
...
| `penetration_trace` | `PenetrationTrace`/null | — | — | **v8**：主动基金持仓穿透记录（前十持仓 + 个股→行业 ETF 加权篮子 + 主代理 + 待验证状态），见 §5.38；无穿透（静态映射型/未穿透型）为 `null`。**键恒在**，禁止省略；`tier=C` 且经穿透得出时非 null（反向不成立，C 亦可为拟用/近似静态代理） |
...
| `no_proxy_count` | integer | 只 | 整数 | **v8**：**无代理只数** = `proxy_code` 为空的持仓只数（穿透失败或无映射者仍计此）；本期实测 `0` |
| `penetrated_count` | integer | 只 | 整数 | **v8**：**经持仓穿透补全只数** = `penetration_trace != null` 的持仓只数（这些持仓的代理由 F10 穿透得出，属 C 档待验证）；本期实测 `2`。口径：`no_proxy_count + penetrated_count ≤ total_count`（穿透补全的持仓此时已带 `proxy_code`，二者不重复计） |
...
**`coverage_tier` 分档判定表（v8 · 覆盖度档位补全口径，唯一判据）**

| 档 | 判定条件（按序命中即停） | 代理来源 | `confidence`（穿透语境） | `action_hint` | `invalidation` |
|---|---|---|---|---|---|
| `A` | 行业 ETF 直接对应（持仓即指数/黄金 ETF 联接，代理与同指数场内 ETF 同向） | 静态映射（直接） | `high` | 信号已可用 | `definition_failure` |
| `B` | 代理 ETF 已映射**且可取到日线**（已通过走前验证） | 静态映射（已验证） | `high`/`mid` | 接线即可用 | `data_failure` |
| `C` | 代理由**拟用/近似**静态映射得出，**或**由**持仓穿透**得出（前十→个股→行业 ETF）；均**未过验证** | 静态（拟用/近似）或穿透 | `mid`/`low`（穿透一律 `low`） | 验证后可用 | `data_failure` |
| `D` | **无映射且未穿透**（F10 前十未取得 / 前十无标的命中「个股→ETF」映射 / 未知码） | 无 | `unknown` | 穿透后可用 | `framework_failure` |

> 口径铁律：① 档位由**命中顺序**唯一确定（A→B→C→D），同一持仓不得同时落两档；② **禁止为提高覆盖率编造代理**——映射不到/穿透不到一律留 `D` 且 `proxy_code=null`；③ `C` 档 `proxy_code` **非 null**（拟用/近似或穿透主代理），仅 `D` 档 `proxy_code=null`（与 §5.1 注一致）；④ 穿透得出必带 `penetration_trace` 且 `status=pending_validation`，**不得当已验证（A/B）消费**；⑤ 本期实测分档 `A2 / B13 / C7 / D0`（D 占 0%，来源 `14-api-impl-report` v8 §0f/§6）。
...
#### 5.35 `Factor`（在册因子全量记录 · v7）

> `FactorItem`（§5.12）是列表投影；`Factor` 是**全量在册记录**（定义 + 状态 + 最新评估摘要），由候选转正或引擎登记后产生。
> 机制正文见 `21-factor-governance.md` §7。

| 字段 | 类型 | 单位 | 精度 | 口径说明 |
|---|---|---|---|---|
| `factor_id` | string | — | 8~32 位 `[a-z0-9_]` | 因子唯一 id |
| `name` | string | — | — | 因子名 |
| `kind` | enum `factor_kind` | — | — | 二元信号 / 打分 |
| `orientation` | enum `factor_orientation` | — | — | 极性 |
| `role_hint` | enum `factor_role_hint` | — | — | 否决位 / 打分位 |
| `horizon` | integer | 交易日 | 整数 | 预测跨度 H ∈ {1,3,15} |
| `inputs` | array[string] | — | — | 声明读取的输入列（闭集，见机制 §1.3） |
| `normalization` | enum `normalization_mode` | — | — | 归一化方式 |
| `expression_hash` | string | — | 16 位 hex | 定义 AST 的规范化哈希（幂等/审计） |
| `role` | enum `factor_role` | — | — | 盘位（正式/淘汰影子/有效影子/冷冻） |
| `pan_label` | string | — | — | 盘位中文（正式盘/淘汰影子盘/有效影子盘/冷冻盘） |
| `status` | enum `factor_status` | — | — | 生命周期状态 |
| `license` | enum `factor_license` | — | — | 牌照（持有态） |
| `license_label` | string | — | — | 牌照中文 |
| `total_score` | number/null | 分 | 4 位 | 最新总分（六窗加权之和）；无评估 null |
| `max_collinearity` | number/null | 相关系数 | 4 位 | 与现役因子最大共线绝对值（F2 度量）；无对手 null |
| `collinear_with` | string/null | — | — | 最高共线的现役因子 id；无 null |
| `delta_pp` | number/null | 百分点 | 4 位 | 加入现配置的组合 Δ 增量；未测 null |
| `open_count` | integer/null | 次 | 整数 | 总开口数（条件绩效分母之一） |
| `independent_trigger_days` | integer/null | 交易日 | 整数 | 独立触发日数（去重叠；绩效/t 一律按此） |
| `sample_days` | integer/null | 交易日 | 整数 | 评估样本天数 |
| `admission_passed` | boolean | — | — | 准入三项 AND 结果（`F1 ∧ F2 ∧ F3`） |
| `death_condition` | string/null | — | — | 采纳当天预注册的死亡条件原文（正式盘必填） |
| `added_at` | string(date)/null | — | — | 入盘日 |
| `last_scored_at` | string(timestamp)/null | — | 秒 | 最近一次评分时间 |
| `note` | string | — | — | 一句话说明 |

#### 5.36 `FactorDefinition`（因子候选提交请求体 · v7）

> `POST /factors/candidates` 的请求体；字段含义与校验见 `21-factor-governance.md` §1.2/§1.6。
> 定义是**声明式纯表达式**，回测器不读其内部实现，只解释 AST。

| 字段 | 类型 | 单位 | 精度 | 口径说明 |
|---|---|---|---|---|
| `factor_id` | string/null | — | 8~32 位 `[a-z0-9_]` | 可选；缺省由服务端生成；重复 → `E_CONFLICT` |
| `name` | string | — | — | 因子名（必填，非空） |
| `kind` | enum `factor_kind` | — | — | 必填 |
| `orientation` | enum `factor_orientation` | — | — | 必填 |
| `role_hint` | enum `factor_role_hint` | — | — | 必填 |
| `horizon` | integer | 交易日 | 整数 | 必填，∈ {1,3,15}，默认 1；决定 purge=H+2 |
| `inputs` | array[string] | — | — | 必填；⊆ 输入列闭集（机制 §1.3），否则 `E_VALIDATION` |
| `expression` | object | — | — | 必填；AST，根节点类型须与 `kind` 一致 |
| `normalization` | enum `normalization_mode` | — | — | 必填；归一化只用训练段（防前视 L2） |
| `params` | array[object] | — | — | 可选，默认 `[]`；元素 `{name, type, default, grid}`；有 `params` 时 `grid` 不得为空 |
| `veto_threshold` | number/null | 随因子 | 4 位 | `role_hint=veto` 时必填；`score` 时 null |
| `note` | string | — | — | 可选 |
| `requested_by` | string | — | — | 可选（`orchestrator`/`architect`/`product-manager`/`human`） |
| `task_id` | string/null | — | — | 可选，外部任务 id |

> **AST 节点**两种形态：叶子 `{column, lag}`（`lag` integer ≥ 0）；运算 `{op, args[]}`（`op` ∈ `expression_op` 穷举）。
> 校验铁律：未声明列、负 `lag`/负 `shift`、非正窗口、未知 `op`、空 `params.grid`、根节点类型不符，任一即 `E_VALIDATION`。

#### 5.37 `Scorecard` / `ScorecardWindow`（回测记分卡 · v7）

> `GET /factors/candidates/{candidate_id}` 的响应 `data`。机制正文见 `21-factor-governance.md` §2~§5。
...
| `factor_id` | string | — | — | 因子 id |
| `name` | string | — | — | 因子名 |
| `status` | enum `candidate_status` | — | — | submitted/backtesting/done/failed |
| `verdict` | enum `backtest_verdict` | — | — | 准入判定；回测未完成时为 null |
| `admission_passed` | boolean | — | — | `F1 ∧ F2 ∧ F3` 结果 |
| `windows` | array[`ScorecardWindow`] | — | — | 固定 6 项（顺序同 `factor_window`） |
| `total_score` | number/null | 分 | 4 位 | 六窗 `k_lift×weight×admission` 之和；全窗无数据 null |
| `bootstrap_percentiles` | object | — | — | `{p50,p90,p95,p99,p995}`；真实引擎产出前返回 null（**不得用 legacy 常量冒充**） |
| `total_score_percentile` | number/null | 分位 | 4 位 | 本因子总分在 bootstrap 零分布中的分位 [0,1]；未校准 null |
| `f1_validity` | object | — | — | `{result: enum dig_result, detail: string, lift_m6: number/null, lift_d60: number/null}` |
| `f2_independence` | object | — | — | `{result: enum dig_result, detail: string, max_collinearity: number/null, collinear_with: string/null}` |
| `f3_increment` | object | — | — | `{result: enum dig_result, detail: string, delta_pp: number/null, open_count_new: integer/null, open_count_base: integer/null, collapse_ratio: number/null}` |
| `license` | enum `factor_license` | — | — | 当前可授予牌照 |
| `license_label` | string | — | — | 牌照中文 |
| `pan` | enum `factor_role`/null | — | — | 建议盘位（准入未过 null） |
| `pan_label` | string/null | — | — | 盘位中文 |
| `max_collinearity` | number/null | 相关系数 | 4 位 | 与现役最大共线绝对值（F2） |
| `collinear_with` | string/null | — | — | 最高共线对手 id |
| `delta_pp` | number/null | 百分点 | 4 位 | 加入现配置的组合 Δ 增量 |
| `open_count` | integer/null | 次 | 整数 | 总开口数 |
| `independent_trigger_days` | integer/null | 交易日 | 整数 | 独立触发日数（去重叠） |
| `sample_days` | integer/null | 交易日 | 整数 | 评估样本天数 |
| `placebo_delta_pp` | number/null | 百分点 | 4 位 | 同频随机入场的 95 分位对照 Δ |
| `placebo_min_pp` | number | 百分点 | 4 位 | 冻结 0.02（真因子须超 placebo ≥2pp） |
| `holdout` | object/null | — | — | 冻结留出块 `{days, delta_pp, hit_rate, baseline_rate, mismatch}`；未评估 null |
| `chosen_params` | object/null | — | — | 训练段选中的参数（冻结进记分卡）；无参数 null |
| `frozen_config_hash` | string | — | 16 位 hex | `FrozenParams` 哈希；变化即旧记分卡不可比 |
| `definition_hash` | string | — | 16 位 hex | 定义 AST 哈希 |
| `cv_mode` | string | — | — | 固定 `walk_forward` |
| `purge_gap` | string | — | — | 固定 `H+2` |
| `holdout_days` | integer | 交易日 | 整数 | 固定 60 |
| `sample_period` | string | — | — | 样本区间，如 `2023-01-01..2026-06-30` |
| `judges` | array[object] | — | — | 四裁判 `{judge, scope, metric, value, note}`；`judge` ∈ `score_judge` |
| `scored_at` | string(timestamp)/null | — | 秒 | 回测完成时间；未完成 null |
| `as_of` | string(date)/null | — | — | 数据截止日 |
...
| `window` | enum `factor_window` | — | — | 六窗 |
| `window_label` | string | — | — | 15日/30日/60日/6月/24月/全样本 |
| `lookback_days` | integer | 交易日 | 整数 | 该窗回看交易日数（近似；边界按真实交易日历） |
| `lift` | number/null | 倍数 | 4 位 | `命中率 ÷ 基线`；无独立触发日 null |
| `hit_rate` | number/null | 比率 | 4 位 | 走前 CV 命中率（按独立触发日聚合） |
| `baseline_rate` | number/null | 比率 | 4 位 | `max(恒涨率,恒跌率)` |
| `delta_pp` | number/null | 百分点 | 4 位 | `hit_rate − baseline_rate` |
| `open_count` | integer | 次 | 整数 | 总开口数 |
| `independent_trigger_days` | integer | 交易日 | 整数 | 去重叠独立触发日数 |
| `n_w` | integer | 交易日 | 整数 | 准入度分母 `N_w = 5/10/15/30/60/120` |
| `admission` | number | 比率 | 4 位 | `min(1, independent_trigger_days / n_w)` |
| `k_lift` | number | 分 | 4 位 | 悬崖查表（≥1.30→1.0 / ≥1.20→0.80 / ≥1.10→0.65 / ≥1.00→0.50 / <1.00→−0.80 / <0.84→−1.3；`lift` null 时 0） |
| `weight` | number | 分 | 4 位 | 窗权重，穷举 `8/22/15/10/30/15`（对应 d15/d30/d60/m6/m24/full，合计 100） |
| `score` | number | 分 | 4 位 | `k_lift × weight × admission` |
| `cliff_hit` | string/null | — | — | 命中悬崖档位标签（如 `≥1.20`）；`lift` null 时 null |
| `t_value` | number/null | — | 3 位 | 独立触发日聚合的 t 值；样本不足 null |
| `low_sample` | boolean | — | — | `open_count < 20`（该窗不出结论） |

#### 5.38 `PenetrationTrace` / `PenetrationTopHolding` / `PenetrationBasket` / `PenetrationUnmapped`（持仓穿透记录 · v8）

> 归属：`HoldingRow.penetration_trace`（§5.1）、`GET /holdings`（§6.2）与 `GET /holdings/{code}`（§6.2）逐行返回。
> 来源与口径：`14-api-impl-report` v8 §0f（`proxy_map.penetrate_rows`）；对**无静态代理的主动全市场基金**抓天天基金 F10 前十持仓，按「个股→行业 ETF」映射加权归集，取权重最高 ETF 为**主代理**。
> **诚实性铁律**：穿透主代理一律 `confidence=low`、`status=pending_validation`（待验证），**不得**宣称已验证；穿透不到仍留 `D` 且 `penetration_trace=null`，**禁止编造代理**。

`PenetrationTrace`：

| 字段 | 类型 | 单位 | 精度 | 口径说明 |
|---|---|---|---|---|
| `method` | enum `penetration_method` | — | — | 穿透算法；**单值** `eastmoney_f10_top10_weighted` |
| `source` | string | — | — | F10 数据源 URL（天天基金 `FundArchivesDatas.aspx?type=jjcc&code=<code>&topline=10`） |
| `report_date` | string(date)/null | — | — | 持仓报告期（F10「截止至」日期）；未取得 null |
| `fetched_at` | string(timestamp) | — | 秒 | 抓取缓存时刻（ISO 8601 带偏移，`Asia/Shanghai`） |
| `top10` | array[`PenetrationTopHolding`] | — | — | 前十持仓明细（升序按 `rank`；上限 10 项） |
| `basket` | array[`PenetrationBasket`] | — | — | 加权篮子，按 `weight_pct` 降序；`basket[0]` 即主代理 |
| `mapped_weight_pct` | number | 百分数 | 2 位 | **已映射标的前十权重合计**（`0.0~100.0`；62.34 = 62.34%）。**见下方单位例外** |
| `unmapped` | array[`PenetrationUnmapped`] | — | — | 未命中「个股→ETF」映射的标的（审计用，可空） |
| `proxy` | string/null | — | 6 位数字 | **主代理** ETF 代码 = `basket[0].etf_code`；无映射 null |
| `proxy_basis` | string | — | — | 主代理选取依据文本（"主代理=前十已映射标的中权重合计最高的 ETF"） |
| `confidence` | enum `confidence` | — | — | 穿透结果置信度；**仅 `low` 或 `unknown`**，不得 `high`/`mid` |
| `status` | enum `penetration_status` | — | — | `pending_validation`（穿透得出）/ `no_mapping`（无标的命中映射）/ `no_report`（前十未取得） |
| `status_label` | string | — | — | `status` 的中文展示文案（后端产出，前端不硬编码；同 `valuation_mode`/`mode_label` 口径） |

`PenetrationTopHolding`（`top10[]` 元素）：

| 字段 | 类型 | 单位 | 精度 | 口径说明 |
|---|---|---|---|---|
| `rank` | integer | — | 整数 | 持仓排名 1~10 |
| `stock_code` | string | — | 6 位数字 | 个股代码 |
| `stock_name` | string | — | — | 个股名称 |
| `weight_pct` | number/null | 百分数 | 2 位 | 该个股占基金净值比（如 8.31 = 8.31%）；F10 未给 null |
| `etf_code` | string/null | — | 6 位数字 | 命中的行业 ETF 代码；未命中 null |
| `etf_name` | string/null | — | — | 命中的行业 ETF 名称；未命中 null |
| `theme` | string/null | — | — | 映射主题说明（如"半导体（兆易创新）"）；未命中 null |

`PenetrationBasket`（`basket[]` 元素）：

| 字段 | 类型 | 单位 | 精度 | 口径说明 |
|---|---|---|---|---|
| `etf_code` | string | — | 6 位数字 | 行业 ETF 代码 |
| `etf_name` | string | — | — | 行业 ETF 名称 |
| `weight_pct` | number | 百分数 | 2 位 | 归集到该 ETF 的前十权重合计（如 53.30 = 53.30%） |
| `stocks` | array[string] | — | 6 位数字 | 贡献该 ETF 权重的个股代码清单 |

`PenetrationUnmapped`（`unmapped[]` 元素）：

| 字段 | 类型 | 单位 | 精度 | 口径说明 |
|---|---|---|---|---|
| `stock_code` | string | — | 6 位数字 | 个股代码 |
| `stock_name` | string | — | — | 个股名称 |
| `weight_pct` | number/null | 百分数 | 2 位 | 该个股前十权重；F10 未给 null |

> **单位例外登记（v8）**：`PenetrationTrace` 家族内 `weight_pct` / `mapped_weight_pct` 中的权重取**百分数**（`8.31` 表示 8.31%），**与 §1.2 `*_pct`「比率小数、展示层 ×100」约定不同**——因该组权重直接来自 F10 页面百分比口径、并由穿透 trace 直出展示。此为**显式登记的 v8 口径例外**，非疏漏；若后续统一为小数 `*_ratio`，须版本 +1 并通知 frontend-dev/backend-dev 双边。`weight_pct` 之外的字段仍全部遵循 §1.2。
...
##### POST /api/v1/factors/candidates ★ v7
- 调用方：**因子页"提交候选"表单（P1）/ 挖掘流程 / 主 Agent 派发的因子研发**；提交因子定义并**触发一次六窗走前回测**（异步任务）。
- 请求体：`FactorDefinition`（§5.36），必填字段见该节；`Content-Type: application/json`。
- 响应 **202** `data`：

| 字段 | 类型 | 单位 | 精度 | 口径说明 |
|---|---|---|---|---|
| `candidate_id` | string | — | — | 服务端生成的候选 id（如 `cand_20260912_153000_ab12`） |
| `factor_id` | string | — | — | 因子 id（请求给了则回显，否则服务端生成） |
| `definition_hash` | string | — | 16 位 hex | 定义 AST 规范化哈希（幂等键） |
| `status` | enum `candidate_status` | — | — | 提交成功恒 `backtesting`（异步） |
| `task_id` | string | — | — | 回测任务 id（供 `GET /tasks/{id}` 轮询） |
| `submitted_at` | string(timestamp) | — | 秒 | 提交时间 |

- 错误：`E_VALIDATION`（AST 校验失败，`detail` 给字段级原因：未声明列 / 负 `shift`/`lag` / 非正窗口 / 未知 `op` / 空 `params.grid` / `kind` 与根节点类型不符 / `veto_threshold` 缺失）/ `E_CONFLICT`（`factor_id` 或 `definition_hash` 已存在；后者直接回旧 `candidate_id` 并 `verdict=duplicate`）/ `E_IO`。
- 口径：① 落候选账本 `factor_candidates.jsonl`（INSERT-ONLY，同 `definition_hash` 幂等）；② 回测为**本地纯 Python 标准库**任务，**不联网**；③ 防前视由回测器强制（机制 §1.6 L1~L7）；④ 本端点**只提交与回测**，不改变任何因子盘位；晋升/淘汰仍由治理周期与状态机（机制 §6）驱动。

##### GET /api/v1/factors/candidates/{candidate_id} ★ v7
- 调用方：**因子页**候选记分卡展开；轮询 `status=backtesting` 直至 `done/failed`。
- 路径参数：`candidate_id`（string）。
- 请求参数（query）：`include_windows`（boolean，可选，默认 true，false 时不返回 `windows` 数组以减小体积）。
- 响应 200 `data`：`Scorecard`（§5.37）。
- 错误：`E_NOT_FOUND`（未知 `candidate_id`）/ `E_IO`。
- 口径：① 六窗 `lift` 无独立触发日时为 **null**，UI 显示"—"，**禁止估填**；② `bootstrap_percentiles` 在真实引擎产出前为 **null**（**不得用 legacy 常量冒充**，机制 §11 R3）；③ `status != done` 时 `verdict=null`、`windows` 各窗为 null；④ `frozen_config_hash` 与当前 `FrozenParams` 不一致时前端须提示"该记分卡基于旧冻结参数，不可直接比较"；⑤ 记分卡写入 `scorecards.jsonl`（INSERT-ONLY），重跑产生新记录不覆盖。
...
- 口径（**v8 追加**）：响应 `data` 含覆盖计数 `no_proxy_count`（`proxy_code` 为空的只数）/ `penetrated_count`（`penetration_trace != null` 的只数），供前端展示 D 档压缩进度；`no_proxy_count + penetrated_count ≤ total_count`。档位判据以 §5.19「`coverage_tier` 分档判定表」为唯一法律；**禁止为提高覆盖率编造代理**，映射不到/穿透不到如实留 `D`（本期实测 `A2/B13/C7/D0`、`no_proxy_count=0`、`penetrated_count=2`）。
- 口径（**v8 追加**）：`GET /holdings` / `GET /holdings/{code}` 每行含 `penetration_trace`（§5.38，静态映射型持仓为 `null`）；穿透结果一律 `confidence ∈ {low, unknown}`、`status=pending_validation`，**不得当已验证（A/B）消费**。
...
| 因子页候选提交/记分卡（P1） | `/factors/candidates`（POST）、`/factors/candidates/{id}` | ✅ v7 |
...
### 12. v7 追加清单（因子候选提交与回测 · 只加不改）

> 本节是 v7 相对 v6 的**逐条追加台账**（为"每来一个新因子怎么回测/准入"提供可复用机制；机制正文见 `docs/01-architecture/21-factor-governance.md`）。
> **不变式**：v7 **未修改/未删除任何既有字段名与类型**；`factor_book` 六本与 `factor_role` 四盘枚举**保持不变**；晋升/淘汰**不**经新端点。

#### 12.1 新增接口（54~55）

| # | 方法 | 路径 | 用途 | 错误码 |
|---|---|---|---|---|
| 54 | POST | `/api/v1/factors/candidates` | 提交 `FactorDefinition` 并触发六窗走前回测（202） | `E_VALIDATION`/`E_CONFLICT`/`E_IO` |
| 55 | GET | `/api/v1/factors/candidates/{candidate_id}` | 取 `Scorecard` | `E_NOT_FOUND`/`E_IO` |

#### 12.2 新增共享对象

| 对象 | 落点 | 说明 |
|---|---|---|
| `Factor` | §5.35 | 在册因子全量记录（`FactorItem` 仍是列表投影） |
| `FactorDefinition` | §5.36 | 候选提交请求体（声明式 AST + 输入列闭集 + 参数） |
| `Scorecard` / `ScorecardWindow` | §5.37 | 六窗 lift/`k_lift`/权重/准入度/总分 + F1/F2/F3 + 牌照/盘位/共线/Δ/样本/placebo/holdout |

#### 12.3 新增枚举（8 组）

见 §1.3「v7 追加枚举」：`factor_kind` / `factor_orientation` / `factor_role_hint` / `candidate_status` / `backtest_verdict` / `normalization_mode` / `param_type` / `expression_op`。全部穷举，无"等"。

#### 12.4 新增存储（独立于六本 UI 账本，不进 `factor_book` 枚举）

- `factor_candidates.jsonl`（候选流水，INSERT-ONLY，按 `definition_hash` 幂等）；
- `scorecards.jsonl`（记分卡流水，INSERT-ONLY，按 `(candidate_id, definition_hash, frozen_config_hash)` 幂等）；
- 理由：保持 `factor_book` 固定 6 本、`/factors/books` 返回数不变，避免破坏既有 UI 契约。

#### 12.5 不改既有字段名（红线）

v7 未增删 v6 任何字段名/类型；`FactorPan`/`FactorItem`/`FrozenParams`/六本账本/四盘枚举的既有字段与语义保持 v6 原样（仅**新增**对象与端点）。`FrozenParams.license_lines[].min_independent_days` **不改名**；其与 legacy"总分线"的语义差异登记为风险（机制 §11 R2），后续如需追加 `min_total_score` 兄弟字段须版本 +1。

### 13. v8 追加清单（覆盖度档位补全与主动基金持仓穿透 · 只加不改）

> 本节是 v8 相对 v7 的**逐条追加台账**（依据 `14-api-impl-report` v8 §0f/§7 V14~V16，逐条纪要见 `20-arch-verdict-r4.md`）。
> **不变式**：v8 **未修改/未删除任何既有字段名、类型或语义**；`coverage_tier` 四档取值、`HoldingRow`/`CoveragePanel` 既有字段保持 v7 原样；追加仅为「新增字段 / 新增共享对象 / 新增枚举 / 新增分档判定表」。

#### 13.1 对既有对象的追加

| 追加内容 | 归属对象 | 前端消费点 | 对应裁定 |
|---|---|---|---|
| `penetration_trace`（`PenetrationTrace`/null） | `HoldingRow` §5.1 | 持仓页穿透行「前十穿透→主代理（待验证）」详情（§7.1） | V14 |
| `no_proxy_count`（integer，只） | `CoveragePanel` §5.19 | 持仓页覆盖度四档「无代理只数」进度 | V15 |
| `penetrated_count`（integer，只） | `CoveragePanel` §5.19 | 持仓页覆盖度四档「穿透补全只数」进度 | V15 |

#### 13.2 新增共享对象

| 对象 | 落点 | 说明 |
|---|---|---|
| `PenetrationTrace` | §5.38 | 主动基金持仓穿透记录（前十 + 加权篮子 + 主代理 + 待验证） |
| `PenetrationTopHolding` / `PenetrationBasket` / `PenetrationUnmapped` | §5.38 | `top10[]` / `basket[]` / `unmapped[]` 元素（键名穷举） |

#### 13.3 新增枚举（2 组）

见 §1.3「v8 追加枚举」：`penetration_method`（单值 `eastmoney_f10_top10_weighted`）、`penetration_status`（`pending_validation`/`no_mapping`/`no_report`）。`penetration_trace.confidence` **复用**既有 `confidence`（不改、不扩）。

#### 13.4 新增分档判定表（口径钉死，非字段变更）

- §5.19 新增「`coverage_tier` 分档判定表」：A 行业 ETF 直接 / B 代理已映射且可取日线 / C 拟用·近似或穿透得出待验证 / D 无映射且未穿透；命中顺序 A→B→C→D 唯一确定，禁止同落两档。
- 销 v2 偏差 **D2**（代理映射未在契约冻结）：v8 将「完整映射 + 穿透」的分类口径写入契约；**映射表本身（`PROXY_MAP`/`STOCK_ETF_MAP`）仍属产品领域配置，不逐条进契约**，契约只冻结档位判据与置信度纪律。

#### 13.5 登记的实现对齐项（非字段改名，需 backend-dev 在下一轮对齐）

| # | 契约形态 | v8 后端实测 | 性质 | 动作 |
|---|---|---|---|---|
| A1 | `penetration_trace.status` 为 enum `penetration_status`（snake_case）+ 兄弟字段 `status_label` | `status` 当前返回中文自由文本（`"待验证"` / `"无映射（待补充个股→ETF 映射）"`） | 枚举纪律不对齐（§1.1/§1.3） | backend-dev 改为输出枚举 token + `status_label`；只加字段、不改 `top10`/`basket` 等既有键 |
| A2 | `penetration_trace.*.weight_pct` 与 `mapped_weight_pct` 单位 = 百分数（§5.38 单位例外） | 已为百分数（`62.34`/`8.31`） | 与 §1.2 `*_pct` 小数约定不同，已**显式登记为例外** | 契约侧登记（本文 §5.38）；统一为 `*_ratio` 须版本 +1 |

#### 13.6 不改既有字段名（红线）

v8 未增删 v7 任何字段名/类型；`HoldingRow`/`CoveragePanel`/`CoverageTier` 的既有字段与语义保持 v7 原样（仅追加 `penetration_trace`/`no_proxy_count`/`penetrated_count`，并新增 §5.19 判定表与 §5.38 对象）。穿透结果的**待验证**语义为裁定红线：任何消费方不得将 `C` 档穿透代理当作已验证代理。
...
- [x] 所有枚举穷举列出（§1.3，共 54 个：v2 22 + v3 19 + v4 3 + v7 8 + v8 2，无"等"）
...
- [x] **v7 机制**：新增 `POST /factors/candidates` + `GET /factors/candidates/{id}` 与 `Factor`/`FactorDefinition`/`Scorecard`/`ScorecardWindow`，补齐 lift 六窗、`k_lift`、权重、准入度、总分、F1/F2/F3、牌照、盘位、共线性、Δ、样本/独立触发日（§5.35~§5.37、§6.8、§12）
- [x] **v7 防前视**：AST 声明输入列闭集 + 禁负 `shift`/因果窗口 + 归一化只用训练段 + purge=H+2 + holdout 60 日 + placebo≥2pp（§5.36、`21-factor-governance` §1.6）
- [x] **v7 不变式**：只加对象/端点/枚举 + 独立存储，**未改任何既有字段名/类型**，未动 `factor_book` 六本与四盘枚举；晋升/淘汰不经新端点；§12 逐条可核
- [x] **v8 V14**：`HoldingRow.penetration_trace`（§5.1）+ 共享对象 `PenetrationTrace`/`PenetrationTopHolding`/`PenetrationBasket`/`PenetrationUnmapped`（§5.38，元素键名穷举）；单位/精度/口径齐全；无穿透为 `null` 且键恒在
- [x] **v8 V15**：`CoveragePanel.no_proxy_count`/`penetrated_count`（§5.19），计数口径 `no_proxy_count + penetrated_count ≤ total_count`；本期实测 0 / 2
- [x] **v8 分档判定表**：§5.19 A/B/C/D 命中顺序唯一确定 + `confidence` 纪律（穿透仅 `low`/`unknown`）+ 「禁编造代理」；销 v2 偏差 D2
- [x] **v8 枚举**：`penetration_method`（单值）/`penetration_status`（3 值）入 §1.3；`penetration_trace.confidence` 复用 `confidence`，不新增
- [x] **v8 单位例外登记**：`penetration_trace.*_pct` 系列为百分数（§5.38）；与 §1.2 差异显式登记，统一须版本 +1
- [x] **v8 不变式**：只加字段/对象/枚举/判定表，**未改任何既有字段名/类型/语义**；§13 逐条可核
...
- **v7 交接（因子候选提交与回测 · 可复用机制）**：
  - **backend-dev**：① 按 `21-factor-governance.md` §9 落地深度模块 `core/factor_eval.py#evaluate(definition, bars_view, frozen_config) -> Scorecard`，复用既有 `gates/ddsm/veto/portfolio` 原语与 `engine.frozen_params()`；② 实现 §6.8 两端点（202 异步 + 记分卡读取），候选/记分卡落 `factor_candidates.jsonl`/`scorecards.jsonl`（INSERT-ONLY、幂等、`frozen_config_hash`）；③ **机制落地后用它跑 KLOW2 下影线否决与拥挤度六维复合的真实六窗回测填盘**，`ScorecardWindow.lift` 未产出前仍为 null（**不得沿用 legacy 示例数字**）；④ AST 校验须实现 §5.36 全部拒绝项。
  - **frontend-dev**：因子页若纳入"提交候选/记分卡"（P1），字段**直读** §5.35~§5.37；六窗 bars 读 `lift`，跑分表读 `k_lift/weight/admission/score`，缺失一律"—"；`bootstrap_percentiles=null` 时显示"待复现"；`frozen_config_hash` 不匹配时提示"基于旧冻结参数"；**不得本地推算/估算**。
  - **qa**：负向断言——未声明列 / 负 `shift` / 非正窗口 / 未知 `op` / 空 `params.grid` 的提交被 `E_VALIDATION` 拒；同 `definition_hash` 重复提交幂等返回旧 `candidate_id` 且 `verdict=duplicate`；`status=backtesting` 时 `verdict=null`、六窗 null；记分卡无删除/编辑端点；`frozen_config_hash` 随冻结参数变化。
  - **product-manager**：核对 `21-factor-governance.md` §5/§7 生命周期与四盘长文与 `02-prd.md` F-14 一致；确认 holdout 60 日与影子盘 60 日文案区分。
  - **dev-lead**：G-BE-02 自本版起以 **v7** 为唯一法律复核；销项以本文 §12 + `21-factor-governance.md` §11 为核对清单。
- **v8 交接（覆盖度档位补全与主动基金持仓穿透）**：
  - **backend-dev**：① 已实现 `penetration_trace`/`no_proxy_count`/`penetrated_count`，**无需改既有键**；仅需按 §13.5 A1 把 `penetration_trace.status` 从中文自由文本改为 enum `penetration_status` token 并补 `status_label`（只加字段），重跑 `python -m unittest discover -s tests -t .`（v8 基线 **100/100**）；② `penetration_trace.confidence` 严格 ∈ {`low`,`unknown`}、`status=pending_validation`，不得出现 `high`/`mid`。
  - **frontend-dev**：① 持仓页穿透行可读 `HoldingRow.penetration_trace`（前十明细 + `basket` 加权篮子 + `proxy` 主代理 + `status_label`「待验证」），**必须带待验证角标**，不得渲染为已验证代理；② 覆盖度四档头部可读 `no_proxy_count`/`penetrated_count`；③ `status` 走 `status_label` 展示，**不得硬编码中文**；④ `tier` 语义未变，既有渲染无需改。
  - **qa**：负向断言——映射不到/穿透不到的未知码必须 `tier=D` 且 `proxy_code=null` 且 `penetration_trace=null`；穿透得出必为 `tier=C`、`confidence != high/mid`、`status=pending_validation`；`no_proxy_count`/`penetrated_count` 与逐行推导一致；禁止出现无映射却带代理的持仓。
  - **dev-lead**：G-BE-02 自本版起以 **v8** 为唯一法律复核；销项以本文 §13 + `20-arch-verdict-r4.md` 为核对清单。
- **残留风险**（每项含责任人与期限）：
  ...
  13. **v7 候选/记分卡存储路径未与 `06-system-arch` 对齐**（机制建议 `factor_candidates.jsonl`/`scorecards.jsonl`，与现有 `ledger/` 读法存在目录归属待定）→ 由 backend-dev 落地时统一并回报，机制不动存储实现（责任人：backend-dev + architect，期限：S3）。
  14. **v7 `FrozenParams.license_lines[].min_independent_days` 语义与 legacy"总分线 50/60/70/80"不同名不同义** → 机制 §5.2 明确"总分线 + 样本下限并行满足"，未追加 `min_total_score` 兄弟字段前**不得**把该字段当总分线消费（责任人：architect，期限：S3）。
  15. **v7 候选/记分卡端点 UI 是否纳入本期**（Brief §9 因子页未含"提交候选"表单）→ 端点先服务挖掘流程/研发；UI 纳入与否由 product-manager 定（责任人：product-manager + architect，期限：S3 前）。
  16. **v8 `penetration_trace.status` 枚举对齐**：后端当前返回中文自由文本，契约已定 `penetration_status` 枚举 + `status_label`（§13.5 A1）→ 待 backend-dev 输出 token 后销项（责任人：backend-dev，期限：S3 收口）。
  17. **v8 穿透映射表（`PROXY_MAP`/`STOCK_ETF_MAP`）不在契约内**：契约只冻结档位判据与置信度纪律，具体个股→ETF 映射属产品领域配置、且穿透主代理**未经走前验证** → 映射/穿透结果变化不触发契约改版，但若把 `C` 档当 `A/B` 消费须版本 +1（责任人：architect + backend-dev，期限：下一轮触达）。
