...
# 接口需求清单 · 极境 ZenithLens 前端（v3 · 契约 v5 · task OC-SHADOW-FE）

> 唯一法律 = `docs/01-architecture/09-api-contract.md` **v5**。本单**不重定义字段**，只按页面逐条引用契约编号与字段名，
...
> v3 方法（task OC-SHADOW-FE）：只做契约「只读消费」——因子上限改读后端 `caps`、删对 `factor_cap` 的裸渲染；
> 台账展示端不再改字段口径，仅核对 20 交易日滚动数据的渲染路径。契约 v5 文本未收录 `caps`（仍写 `factor_cap`），
> 故按「缺字段写进本单」纪律在 **§10 缺口 10** 登记，请架构师裁定后版本 +1。
...
6. mock 现状：后端未就绪期间由 `static/js/mock/mock.js` 提供替身，**默认关闭**（仅 `?mock=1` 或 `localStorage.zl_mock='1'`）；
   mock 已补齐 v4/v5 字段；对接真实后端时视图层零改动。
7. **v3 增量（因子上限口径）**：因子盘头部上限改为消费后端 **`caps: {veto_max, score_max}`**（`FactorPan`），
   渲染「否决位 ≤4 · 打分位 ≤3」；`caps` 缺失时不显示任何单个上限数字，**彻底删除**对 `factor_cap` 的裸渲染（原 `factors.js:64-66` 误导为「上限 3」）。
   真实 `caps` 已实测（`docs/02-frontend/evidence/api_factors_pans.json`：`{"veto_max":4,"score_max":3}`）。
...
| 编号 | 接口 | 时机 | 需要字段（v4） | 前端四态处理 |
|---|---|---|---|---|
| F1 | `GET /factors/pans` §6.8/§5.11/§5.12 | 进入 Tab | `pans[]`（4 项）`{role,role_label,description,factor_count,factors[],rule_text,pan_status,flow_kind,updated_at,caps{veto_max,score_max},nomination_limit}`（`caps` 为 v3 追加；`factor_cap` 已弃用，后端暂并存，前端只读 `caps`）；`factors[].{factor_id,name,role,role_label,score,total_score,status,license,license_label,lift_bars[6],independent_trigger_days,death_condition,note,ab_progress,added_at}` | 加载=盘骨架；成功=`rule_text` 原样渲染、`lift_bars` 驱动 bars（只画长度，不重算 lift）、总分/牌照/角色 tag；空盘「（当前为空）」；失败=「因子账本读取失败」+重试；超时=同失败 |
...
10. **`FactorPan.caps` 未入契约 v5（v3 新增，task OC-SHADOW-FE）**：因子盘真实上限是**否决位 ≤4 · 打分位 ≤3**，
    而契约 §5.11 `FactorPan` 仅有 `factor_cap`（integer/null，语义被前端误读为单一「上限 3」）。后端已改为返回
    `caps: {veto_max: 4, score_max: 3}`（非正式盘 `caps: null`），实测见 `evidence/api_factors_pans.json`。
    前端只读 `caps`、不再读 `factor_cap`。**请求架构师**：① 在 §5.11 `FactorPan` 正式收录 `caps`（元素 `veto_max`/`score_max`，integer，
     非正式盘 null）；② 标注 `factor_cap` 为 deprecated（后端暂并存以兼容旧壳，下个版本删除）。裁定后契约版本 +1。
11. **`EngineStatus.shadow_status` 语义变更（task OC-SHADOWSTAT-FE / 后端 OC-SHADOWSTAT-BE）**：契约 §6.9 现注 `shadow_status` 为
    `enum shadow_review_status`（`idle/ok/bad`），但后端已改为**记录进度语义** `not_started/recording/qualified/gap`，
    并把偏差审查独立为新增字段 `shadow_audit_status`（`enum shadow_review_status`，`idle/ok/bad`）。实测（`evidence/api_engine_status.json`）：
    `shadow_status="recording"`、`shadow_audit_status="idle"`、`shadow_consecutive_days=20`、`shadow_target_days=60`。
    前端 HUD 已按新语义渲染（进度用 `shadow_status`，偏差用 `shadow_audit_status`，二者不得混用）。**请求架构师**：
    ① 将 §6.9 `shadow_status` 改为 `enum shadow_progress_status`；② 正式收录 `shadow_audit_status`（`enum shadow_review_status`）；
    ③ 保留 `shadow_consecutive_days`/`shadow_target_days`（`N / 60` 副文案）。裁定后契约版本 +1。

## 自验收

- [x] 每个接口写明「调用页面/时机 + 用途 + 需要字段 + 前端如何四态处理」
- [x] 字段名全部引用 `09-api-contract.md` **v5**（逐字一致），未自造字段；`caps` 为后端已实现但契约未收录项，已登记 §10 缺口 10
- [x] v4 增量字段（A-07/A-08/A-09/A-10）逐条落到消费点（§0）；v3 `caps` 落到 `factors.js#renderPans`
- [x] 契约缺口集中列出（§10，共 11 条，需架构师裁定）
- [x] 未修改任何契约/架构文件；未写 Python 业务代码；未改 `runtime/**`；未派发子任务

## 下游交接

- **主消费方**：`backend-dev`。需其执行：按 §1~§8 逐接口实现，字段名与 v4 逐字一致（G-BE-02 红线）；**v4 追加字段必带**（`weighting_basis`/`weighting_policy_note`、`production_layer`/`research_layer`/`default_layer`/`dual_view_note`、`color_scheme`/`color_token_prefix`、`retention_policy`/`first_of_day_immutable`/`archive_manifest_at`、`FrozenParams.weighting_basis`）；**v3 追加字段 `caps{veto_max,score_max}` 必带**（非正式盘 `caps=null`），`factor_cap` 保持并存直至契约 +1；`created_at` INSERT-ONLY 且与 `written_at` 恒等；立方体空样本格显式返回 `naive_p=null`、`color_band='no_sample'`。
