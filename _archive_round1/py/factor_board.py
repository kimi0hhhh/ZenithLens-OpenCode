...
PAN_META = {
    "adopted": {
        ...
        "description": "当前生产因子（否决位 ≤4、打分位受状态空间约束 ≤3）",
        "factor_cap": 3, "nomination_limit": None, "pan_status": "on",
        "rule_text": ("正式盘：仅收已过准入三项（F1 有效性 / F2 独立性 / F3 增量）AND 的因子；"
                      "总数上限受否决位 ≤4、打分位 ≤3 约束；每月 1 日月榜跑留一法，"
...
def score_detail(factor_id):
    ...
    return {
        ...
        "windows": out_windows,
        "bootstrap_percentiles": BOOTSTRAP_PERCENTILES,
        "judges": [dict(j, value=None) for j in score_chain()["judges"]],
