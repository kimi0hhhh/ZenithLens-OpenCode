    ...
    for vr_layer in range(5):
        ...
        for r2 in range(5):
            for om in range(5):
                ...
                status = "filled" if (np_ is not None or dn > 0) else "empty"
                if status == "filled":
                    filled += 1
                cells.append({
                    ...
                    "ddsm_n": dn, "ddsm_p": round(dp_, 4) if dp_ is not None else None,
                    "color_band": _color_band(np_ if np_ is not None else dp_),
                })
        layers.append({
...
def frozen_params():
    return {
        ...
        "license_lines": [
            {"license": "candidate", "min_independent_days": 50, "note": "只记账不参与生产"},
